...
// This subscription allows only one Container Apps environment total, and that slot
// is already used by thinkschool-env (created in the earlier manual az containerapp
// exercise). Reference it directly instead of provisioning a new one - Container Apps
// and their environment don't need to share a resource group, only a region.
...
// Named 'quotes-api-azd' rather than 'quotes-api' because Container App names must be
// unique within an environment (not just within a resource group), and 'quotes-api'
// already exists in thinkschool-env from the earlier manual az containerapp exercise.
// The 'azd-service-name: quotes-api' tag below is what maps this resource back to the
// 'quotes-api' service in azure.yaml, so azd's service-to-resource resolution is
// unaffected by the actual Azure resource name.
